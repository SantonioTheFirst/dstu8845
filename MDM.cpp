...
uint32_t run_mock_cipher(const uint64_t* state) {
    ...
    // Берем два первых слова состояния для инициализации регистра
    uint32_t s0 = static_cast<uint32_t>(state[0]);
    uint32_t s1 = static_cast<uint32_t>(state[1] >> 32);

    for (int r = 0; r < TOTAL_ROUNDS; ++r) {
        // ARX-перемешивание (Addition, Rotation, XOR) для быстрого роста степени
        s0 ^= (s1 * (s1 | 1)) ^ r;       // Нелинейный шаг
        s1 = (s1 << 5) | (s1 >> 27);     // Циклический сдвиг
        s1 += s0;                        // Сложение с переносом
        
        // Экстрагируем старший бит как нелинейный выход текущего раунда
        uint32_t round_bit = (s0 >> 31) & 1;
        
        // Упаковываем бит раунда в итоговую сигнатуру
